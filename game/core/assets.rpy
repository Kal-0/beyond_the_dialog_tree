...
init -1 python:
    def append_to_custom_log(who, what):
        speaker = who if who else "Narração"
        clean_text = renpy.filter_text_tags(what, allow=[])
        entry = f"{speaker}: {clean_text}"
        if not store.custom_log or store.custom_log[-1] != entry:
            store.custom_log.append(entry)
